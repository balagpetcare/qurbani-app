...
type Props = { caseHistoryId: number };

export function AdminCaseHistoryModerationControls({ caseHistoryId }: Props) {
  ...
  async function approve() {
    ...
    setBusy(true);
    try {
      const res = await fetch(`/api/admin/moderation/case-histories/${caseHistoryId}/approve`, {
        method: "POST",
  ...
  async function reject(e: React.FormEvent) {
    ...
    setBusy(true);
    try {
      const res = await fetch(`/api/admin/moderation/case-histories/${caseHistoryId}/reject`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
